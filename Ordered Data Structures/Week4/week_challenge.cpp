...
void downHeap(Node *n){
   Node *min_child_node = min_child( n );
  
  if( min_child_node!=nullptr )
  {
      if( min_child_node->value<n->value )
      {
        
        int temp = min_child_node->value;
        min_child_node->value = n->value;
        n->value = temp;
        
        downHeap(min_child_node);
      }
      else
      {
        return;
